...
exports.setUserAsAdmin = async (req, res) => {
    try {
        ...
        if (!req.user.isAdmin) {
            return res.status(403).send({ message: 'Permission denied. Only admins can set users as admins.' });
        }
    
        user.isAdmin = true;
        await user.save();
        res.status(200).send({ message: 'User has been set as admin successfully' });
    } catch (error) {
        console.error(error);
        res.status(500).send({ message: 'Server error' });
    }
